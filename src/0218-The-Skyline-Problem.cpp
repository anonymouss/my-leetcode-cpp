...
public:
    std::vector<std::vector<int>> getSkyline(std::vector<std::vector<int>> &buildings) {
        std::multiset<std::pair<int, int>> bottoms; // ordered
        std::vector<std::vector<int>> skyline;

        for (auto &b : buildings) {
            bottoms.emplace(std::make_pair(b[0], -b[2]));    // left bottom
            bottoms.emplace(std::make_pair(b[1], b[2]));     // right bottom
        }
        ...
        for (auto &bottom : bottoms) {
            if (bottom.second < 0) {
                // left bottom
                heights.emplace(-bottom.second);
            } else {
                // right bottom
                heights.erase(heights.find(bottom.second));
            }
            auto currentHeight = *heights.rbegin();
            if (currentHeight != lastHeight) {
                skyline.emplace_back(std::vector<int>{bottom.first, currentHeight});
...
int main() {
    ...
    auto print_vv = [&print_v](const T &vvs) {
        std::cout << "[";
        for (const auto &vs : vvs) print_v(vs);
        std::cout << "]" << std::endl;
    };

    for (auto &buildings : buildingsv) {
        TestCount();
        print_vv(sln.getSkyline(buildings));
    }
}